...
def scp_files(files_location):
    hosts=get_nodes_ID()
    for h in hosts:
        print_header(bcolors.OKCYAN, '', h)
        print("scp -r {origin_path} {host}:{destination_path}".format(origin_path=files_location[0], destination_path=files_location[1], host=h))
        print(subprocess_cmd("scp -r {origin_path} {host}:{destination_path}".format(origin_path=files_location[0], destination_path=files_location[1], host=h), 'local'))
